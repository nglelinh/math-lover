---
layout: post
title: 06-04 Hình học - Hình chữ nhật và hình bình hành
chapter: '08'
order: 4
owner: Math Lover Team
lang: vi
categories:
- chapter08
lesson_type: required
---
Trong bài học này, chúng ta sẽ khám phá hai hình học đặc biệt - hình chữ nhật và hình bình hành!
![Hinh chu nhat hinh binh hanh]({{ site.baseurl }}/img/chapter_img/chapter06/06_rectangle_parallelogram.svg)


![Hình tam giác](https://upload.wikimedia.org/wikipedia/commons/thumb/e/e9/Triangle.svg/960px-Triangle.svg.png)

*Ảnh: Wikimedia Commons — sơ đồ toán học*

---

## Mục tiêu

Trong bài học này, em sẽ:

- Nhận biết hình chữ nhật
- Nhận biết hình bình hành
- Tính chu vi và diện tích

---

## Kiến thức đã biết

Em đã biết về hình vuông và hình tam giác. Hôm nay, chúng ta sẽ học thêm về các hình mới nhé!

![Hình vuông — bốn cạnh bằng nhau](https://upload.wikimedia.org/wikipedia/commons/thumb/c/c9/Square_%28geometry%29.svg/960px-Square_%28geometry%29.svg.png)

*Ảnh: Wikimedia Commons — sơ đồ toán học*


---

## Khám phá toán học

### Hình chữ nhật

**Đặc điểm:**
- 4 góc đều là góc vuông (90°)
- 2 cạnh dài bằng nhau
- 2 cạnh ngắn bằng nhau
- Có 2 đường chéo bằng nhau

**Chu vi:** C = (dài + rộng) × 2

**Diện tích:** S = dài × rộng

**Ví dụ:** Hình chữ nhật có chiều dài 8cm, chiều rộng 5cm
- Chu vi = (8 + 5) × 2 = 26cm
- Diện tích = 8 × 5 = 40cm²

### Hình bình hành

**Đặc điểm:**
- 2 cặp cạnh đối song song
- 2 cặp cạnh đối bằng nhau
- Các góc đối bằng nhau

**Chu vi:** C = (cạnh a + cạnh b) × 2

**Diện tích:** S = cạnh đáy × chiều cao

---

## Hoạt động khám phá


<!-- vi-interactive:start -->
{% include vi-interactive-lesson.html %}
<!-- vi-interactive:end -->
### Hoạt động 1: Nhận biết hình
Trong các hình sau, hình nào là hình chữ nhật, hình nào là hình bình hành?



![Hình tròn — tất cả điểm cách tâm bằng nhau](https://upload.wikimedia.org/wikipedia/commons/thumb/9/9e/Circle.svg/960px-Circle.svg.png)

*Ảnh: Wikimedia Commons — sơ đồ toán học*

### Hoạt động 2: Tính chu vi và diện tích

Tính chu vi và diện tích hình chữ nhật:
- Chiều dài 10cm, chiều rộng 6cm
- Chu vi = (10 + 6) × 2 = 32cm
- Diện tích = 10 × 6 = 60cm²

### Hoạt động 3: Vẽ hình

Vẽ hình bình hành có cạnh 5cm và 3cm, chiều cao 4cm. Tính diện tích.

---

## Toán học qua các thời đại

### 🕰️ Hình học trong lịch sử

**Euclid** (300 TCN) - nhà toán học Hy Lạp - đã viết cuốn "Cơ sở" về hình học, sách toán quan trọng nhất lịch sử!

**Người Ai Cập** dùng hình chữ nhật để đo đất đai sau mỗi mùa lũ!

### 📜 Hình học trong cuộc sống

**Kiến trúc:** Nhà cửa, cửa sổ
**Thiết kế:** Bàn, ghế, tranh
**Mẫu vải:** Hoa văn hình bình hành

---

## Bài tập luyện tập

### Bài 1: Tính chu vi và diện tích hình chữ nhật

a) 7cm × 4cm
b) 12cm × 8cm

### Bài 2: Tính diện tích hình bình hành

a) Cạnh đáy 10cm, chiều cao 5cm
b) Cạnh đáy 8cm, chiều cao 6cm

### Bài 3: Trắc nghiệm

Hình chữ nhật có:
   A. 4 cạnh bằng nhau
   B. 2 cặp cạnh bằng nhau
   C. 3 cạnh bằng nhau

---

## Góc cha mẹ

### Điều em đang khám phá

Con đang học về **hình chữ nhật và hình bình hành** - hai hình rất phổ biến trong cuộc sống!

### Gợi ý thảo luận

- Tìm các hình trong nhà
- Đo kích thước đồ vật
- Vẽ và tính diện tích

### Kỹ năng phát triển

- Quan sát hình dạng
- Tính toán diện tích
- Tư duy không gian
