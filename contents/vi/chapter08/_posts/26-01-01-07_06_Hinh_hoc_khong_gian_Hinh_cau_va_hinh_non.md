---
layout: post
title: 06-11 Hình học không gian - Hình cầu và hình nón
chapter: '08'
order: 11
owner: Math Lover Team
lang: vi
categories:
- chapter08
lesson_type: required
---
Trong bài học này, chúng ta sẽ khám phá hai hình không gian thú vị - hình cầu và hình nón!
![Hinh cau hinh non]({{ site.baseurl }}/img/chapter_img/chapter07/06_sphere_cone.svg)


![Hình tam giác](https://upload.wikimedia.org/wikipedia/commons/thumb/e/e9/Triangle.svg/960px-Triangle.svg.png)

*Ảnh: Wikimedia Commons — sơ đồ toán học*

---

## Mục tiêu

Trong bài học này, em sẽ:

- Nhận biết hình cầu và hình nón
- Tính thể tích hình cầu
- Tính thể tích hình nón

---

## Kiến thức đã biết

Em đã biết về hình hộp chữ nhật và hình lập phương. Hôm nay, chúng ta sẽ học thêm về các hình mới nhé!

![Hình vuông — bốn cạnh bằng nhau](https://upload.wikimedia.org/wikipedia/commons/thumb/c/c9/Square_%28geometry%29.svg/960px-Square_%28geometry%29.svg.png)

*Ảnh: Wikimedia Commons — sơ đồ toán học*


---

## Khám phá toán học

### Hình cầu

**Đặc điểm:**
- Tất các điểm trên mặt cầu cách tâm một khoảng bằng nhau
- Không có đỉnh, không có cạnh

**Ví dụ:** Quả bóng, viên bi, Trái Đất

**Thể tích hình cầu:**
```
V = (4/3) × π × r³
```

**Ví dụ:** Bán kính 3cm
V = (4/3) × 3.14 × 3³
= (4/3) × 3.14 × 27
= 113.04 cm³

### Hình nón

**Đặc điểm:**
- Có 1 đỉnh nhọn
- Có 1 mặt đáy là hình tròn
- Có 1 mặt xung quanh cong

**Ví dụ:** Mũi dùi, nón lá, que kem

**Thể tích hình nón:**
```
V = (1/3) × π × r² × h
```

**Ví dụ:** Bán kính 2cm, chiều cao 6cm
V = (1/3) × 3.14 × 2² × 6
= (1/3) × 3.14 × 4 × 6
= 25.12 cm³

---

## Hoạt động khám phá


<!-- vi-interactive:start -->
{% include vi-interactive-lesson.html %}
<!-- vi-interactive:end -->
### Hoạt động 1: Nhận biết hình
Tìm các đồ vật có dạng hình cầu và hình nón trong nhà.



![Hình tròn — tất cả điểm cách tâm bằng nhau](https://upload.wikimedia.org/wikipedia/commons/thumb/9/9e/Circle.svg/960px-Circle.svg.png)

*Ảnh: Wikimedia Commons — sơ đồ toán học*

### Hoạt động 2: Tính thể tích hình cầu

Tính thể tích hình cầu có bán kính 6cm.

**Giải:** V = (4/3) × 3.14 × 6³ = 904.32 cm³

### Hoạt động 3: Tính thể tích hình nón

Tính thể tích hình nón có bán kính 3cm, chiều cao 9cm.

**Giải:** V = (1/3) × 3.14 × 3² × 9 = 84.78 cm³

---

## Toán học qua các thời đại

### 🕰️ Hình cầu trong lịch sử

**Archimedes** (287-212 TCN) là người đầu tiên tìm ra công thức tính thể tích hình cầu!

**Eratosthenes** đã đo được bán kính Trái Đất cách đây 2.200 năm!

### 📜 Ứng dụng hiện đại

**Bóng đá:** Quả bóng là hình cầu
**Kiến trúc:** Mái vòm
**Khoa học:** Viên nén, viên bi

---

## Bài tập luyện tập

### Bài 1: Tính thể tích hình cầu

a) r = 2cm
b) r = 5cm

### Bài 2: Tính thể tích hình nón

a) r = 4cm, h = 9cm
b) r = 3cm, h = 12cm

---

## Góc cha mẹ

### Điều em đang khám phá

Con đang học về **hình cầu và hình nón** - hai hình không gian thú vị!

### Gợi ý thảo luận

- Tìm các đồ vật hình cầu và hình nón
- Thực hành với đất nặn
- Liên hệ với các đồ vật trong nhà
