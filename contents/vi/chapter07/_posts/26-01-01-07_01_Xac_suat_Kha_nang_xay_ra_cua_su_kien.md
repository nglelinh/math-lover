---
layout: post
title: '08-02 Xác suất - Khả năng xảy ra của sự kiện'
chapter: '07'
order: 2
owner: Math Lover Team
lang: vi
categories:
- chapter07
lesson_type: required
---
Trong bài học này, chúng ta sẽ khám phá thế giới của xác suất - cách đo lường khả năng xảy ra của các sự kiện!
![Xac suat]({{ site.baseurl }}/img/chapter_img/chapter07/01_probability.svg)


![Xúc xắc — thí nghiệm xác suất](https://upload.wikimedia.org/wikipedia/commons/thumb/8/82/Dice.svg/960px-Dice.svg.png)

*Ảnh: Wikimedia Commons — sơ đồ toán học*

---

## Mục tiêu

Trong bài học này, em sẽ:

- Hiểu khái niệm xác suất
- Tính xác suất đơn giản
- Ứng dụng xác suất trong cuộc sống

---

## Kiến thức đã biết

Em đã biết về phân số và tỉ số. Hôm nay, chúng ta sẽ học cách đo lường khả năng xảy ra nhé!

![Biểu đồ cột — so sánh dữ liệu](https://upload.wikimedia.org/wikipedia/commons/thumb/4/49/Bar_chart.svg/960px-Bar_chart.svg.png)

*Ảnh: Wikimedia Commons — sơ đồ toán học*


---

## Khám phá toán học

### Xác suất là gì?

**Xác suất** là số đo khả năng xảy ra của một sự kiện.

**Công thức:**
```
Xác suất = Số kết quả thuận lợi / Tổng số kết quả có thể
```

**Ví dụ:** Tung đồng xu
- Có 2 mặt: Sấp (S) và Ngửa (N)
- Xác suất ra mặt Ngửa = 1/2 = 50%

### Ví dụ về xác suất

**Ví dụ 1:** Gieo một con xúc xắc
- Có 6 mặt (1, 2, 3, 4, 5, 6)
- Xác suất ra mặt 3 = 1/6

**Ví dụ 2:** Rút một lá bài từ bộ bài 52 lá
- Xác suất rút được lá Át = 4/52 = 1/13

---

## Hoạt động khám phá


<!-- vi-interactive:start -->
{% include vi-interactive-lesson.html %}
<!-- vi-interactive:end -->
### Hoạt động 1: Tung đồng xu
Tung đồng xu 10 lần và ghi lại kết quả:
- Mặt Sấp: ... lần
- Mặt Ngửa: ... lần



![Tam giác Pascal — mẫu hình số](https://upload.wikimedia.org/wikipedia/commons/thumb/4/4b/Pascal_triangle.svg/960px-Pascal_triangle.svg.png)

*Ảnh: Wikimedia Commons — sơ đồ toán học*

### Hoạt động 2: Gieo xúc xắc

Gieo xúc xắc 12 lần và đếm:
- Số lần ra mặt chẵn: ...
- Số lần ra mặt lẻ: ...

### Hoạt động 3: Tính xác suất

Trong hộp có 3 quả bóng đỏ và 2 quả bóng xanh. Lấy ngẫu nhiên 1 quả. Tính xác suất lấy được bóng đỏ?

**Giải:** 
- Tổng: 5 quả
- Bóng đỏ: 3 quả
- Xác suất = 3/5

---

## Toán học qua các thời đại

### 🕰️ Xác suất trong lịch sử

**Pierre-Simon Laplace** (1749-1827) - nhà toán học Pháp - được gọi là "cha đẻ của xác suất"!

**Blaise Pascal** (1623-1662) và **Fermat** đã phát triển lý thuyết xác suất khi giải bài toán đánh bạc!

### 📜 Ứng dụng hiện đại

**Thời tiết:** Dự báo 70% trời mưa
**Y học:** Thử nghiệm thuốc
**Bảo hiểm:** Tính phí bảo hiểm

---

## Bài tập luyện tập

### Bài 1: Tính xác suất

a) Gieo xúc xắc, xác suất ra mặt 5?
b) Rút bài, xác suất rút được lá J?

### Bài 2: Trắc nghiệm

Trong hộp có 4 bóng đỏ, 6 bóng xanh. Xác suất rút được bóng đỏ?
   A. 4/10   B. 6/10   C. 2/10

---

## Góc cha mẹ

### Điều em đang khám phá

Con đang học về **xác suất** - cách đo lường khả năng xảy ra của sự kiện!

### Gợi ý thảo luận

- Chơi trò tung đồng xu để hiểu xác suất
- Xem dự báo thời tiết và thảo luận về xác suất
- Tìm ví dụ xác suất trong cuộc sống
