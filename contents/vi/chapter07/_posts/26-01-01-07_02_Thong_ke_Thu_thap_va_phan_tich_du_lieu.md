---
layout: post
title: '08-05 Thống kê - Thu thập và phân tích dữ liệu'
chapter: '07'
order: 5
owner: Math Lover Team
lang: vi
categories:
- chapter07
lesson_type: required
---
Trong bài học này, chúng ta sẽ học cách thu thập và phân tích dữ liệu như các nhà khoa học!
![Thong ke]({{ site.baseurl }}/img/chapter_img/chapter07/02_statistics.svg)


![Biểu đồ cột — so sánh dữ liệu](https://upload.wikimedia.org/wikipedia/commons/thumb/4/49/Bar_chart.svg/960px-Bar_chart.svg.png)

*Ảnh: Wikimedia Commons — sơ đồ toán học*

---

## Mục tiêu

Trong bài học này, em sẽ:

- Hiểu khái niệm thống kê
- Thu thập dữ liệu
- Phân tích và trình bày dữ liệu

---

## Kiến thức đã biết

Em đã biết về biểu đồ và số liệu. Hôm nay, chúng ta sẽ học sâu hơn về thống kê nhé!

![Biểu đồ tròn — phần trăm](https://upload.wikimedia.org/wikipedia/commons/thumb/a/a5/Pie_chart.svg/960px-Pie_chart.svg.png)

*Ảnh: Wikimedia Commons — sơ đồ toán học*


---

## Khám phá toán học

### Thống kê là gì?

**Thống kê** là khoa học về thu thập, phân tích và trình bày dữ liệu.

### Các khái niệm cơ bản

**Tần số:** Số lần một giá trị xuất hiện

**Trung bình cộng:** Giá trị trung bình của các số
```
Trung bình = Tổng / Số lượng
```

**Trung vị:** Giá trị ở giữa khi sắp xếp theo thứ tự

**Mốt:** Giá trị xuất hiện nhiều nhất

### Ví dụ

Điểm kiểm tra của 5 bạn: 7, 8, 6, 9, 8
- Trung bình = (7+8+6+9+8)/5 = 38/5 = 7.6
- Trung vị = 8 (sắp xếp: 6, 7, 8, 8, 9)
- Mốt = 8 (xuất hiện 2 lần)

---

## Hoạt động khám phá


<!-- vi-interactive:start -->
{% include vi-interactive-lesson.html %}
<!-- vi-interactive:end -->
### Hoạt động 1: Thu thập dữ liệu
Khảo sát chiều cao (cm) của 10 bạn trong lớp:
145, 150, 142, 155, 148, 151, 147, 153, 149, 146



![Xúc xắc — thí nghiệm xác suất](https://upload.wikimedia.org/wikipedia/commons/thumb/8/82/Dice.svg/960px-Dice.svg.png)

*Ảnh: Wikimedia Commons — sơ đồ toán học*

### Hoạt động 2: Tính trung bình

Tính chiều cao trung bình của 10 bạn trên.

**Giải:** 
Tổng = 145+150+142+155+148+151+147+153+149+146 = 1486
Trung bình = 1486/10 = 148.6 cm

### Hoạt động 3: Tìm trung vị và mốt

Chiều cao đã sắp xếp: 142, 145, 146, 147, 148, 149, 150, 151, 153, 155
- Trung vị = (148+149)/2 = 148.5
- Mốt = không có (mỗi giá trị xuất hiện 1 lần)

---

## Toán học qua các thời đại

### 🕰️ Thống kê trong lịch sử

**John Graunt** (1620-1674) - người Anh - được gọi là "cha đẻ của thống kê"!

**Florence Nightingale** (1820-1910) - nữ điều dưỡng - dùng thống kê để cải thiện y tế!

### 📜 Ứng dụng hiện đại

**Kinh tế:** Tính GDP, lạm phát
**Thể thao:** Thống kê cầu thủ
**Internet:** Phân tích hành vi người dùng

---

## Bài tập luyện tập

### Bài 1: Tính trung bình

Điểm số: 8, 7, 9, 6, 8, 7
Tính trung bình cộng?

### Bài 2: Tìm trung vị

Số liệu: 12, 15, 14, 18, 16, 14
Tìm trung vị?

---

## Góc cha mẹ

### Điều em đang khám phá

Con đang học về **thống kê** - cách thu thập và phân tích dữ liệu!

### Gợi ý thảo luận

- Cùng con thu thập dữ liệu về gia đình
- Xem thống kê thể thao
- Thực hành với bảng tính đơn giản
