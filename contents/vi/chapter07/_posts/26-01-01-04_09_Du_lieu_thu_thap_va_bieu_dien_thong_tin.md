---
layout: post
title: '08-03 Dữ liệu - Thu thập và biểu diễn thông tin'
chapter: '07'
order: 3
owner: Math Lover Team
lang: vi
categories:
- chapter07
lesson_type: required
---
Trong bài học này, chúng ta sẽ học cách thu thập và biểu diễn thông tin như các nhà khoa học!
![Du lieu thu thap va bieu dien thong tin]({{ site.baseurl }}/img/chapter_img/chapter04/04_09_du_lieu_thu_thap_va_bieu_dien_thong_tin.svg)


![Biểu đồ cột — so sánh dữ liệu](https://upload.wikimedia.org/wikipedia/commons/thumb/4/49/Bar_chart.svg/960px-Bar_chart.svg.png)

*Ảnh: Wikimedia Commons — sơ đồ toán học*

---

## Mục tiêu

Trong bài học này, em sẽ:

- Thu thập dữ liệu đơn giản
- Vẽ biểu đồ cột
- Đọc và hiểu biểu đồ

---

## Kiến thức đã biết

Em đã biết đếm và gom nhóm các đồ vật. Hôm nay, chúng ta sẽ học cách tổ chức thông tin nhé!

![Biểu đồ tròn — phần trăm](https://upload.wikimedia.org/wikipedia/commons/thumb/a/a5/Pie_chart.svg/960px-Pie_chart.svg.png)

*Ảnh: Wikimedia Commons — sơ đồ toán học*


---

## Khám phá toán học

### Thu thập dữ liệu

**Ví dụ:** Khảo sát môn thể thao yêu thích của các bạn trong lớp

| Môn thể thao | Số bạn |
|--------------|--------|
| Bóng đá      | 10     |
| Bóng rổ      | 6      |
| Bơi lội      | 4      |
| Cầu lông     | 5      |

### Biểu đồ cột

Biểu đồ cột giúp chúng ta nhìn thấy thông tin nhanh chóng!

```
Bóng đá:    ██████████ (10)
Bóng rổ:    ██████ (6)
Bơi lội:    ████ (4)
Cầu lông:   █████ (5)
```

### Số liệu thống kê

- **Tổng cộng:** 10 + 6 + 4 + 5 = 25 bạn
- **Nhiều nhất:** Bóng đá (10 bạn)
- **Ít nhất:** Bơi lội (4 bạn)

---

## Hoạt động khám phá


<!-- vi-interactive:start -->
{% include vi-interactive-lesson.html %}
<!-- vi-interactive:end -->
### Hoạt động 1: Khảo sát màu sắc
Em hãy hỏi 10 bạn xem bạn thích màu gì và ghi lại:
- Đỏ: ... bạn
- Xanh: ... bạn
- Vàng: ... bạn
- Tím: ... bạn



![Xúc xắc — thí nghiệm xác suất](https://upload.wikimedia.org/wikipedia/commons/thumb/8/82/Dice.svg/960px-Dice.svg.png)

*Ảnh: Wikimedia Commons — sơ đồ toán học*

### Hoạt động 2: Vẽ biểu đồ

Dựa vào kết quả khảo sát, em hãy vẽ biểu đồ cột nhé!

### Hoạt động 3: Trả lời câu hỏi

- Màu nào được yêu thích nhất?
- Màu nào ít được yêu thích nhất?
- Tổng có bao nhiêu bạn được hỏi?

---

## Toán học qua các thời đại

### 🕰️ Thống kê trong lịch sử

**John Graunt** (1620-1674) - người Anh - được gọi là "cha đẻ của thống kê" vì đã phân tích dữ liệu dân số London!

### 📜 Dữ liệu trong chiến tranh

Trong **Thế chiến II**, thống kê giúp các nước:
- Dự đoán số máy bay bị bắn hạ
- Lập kế hoạch cung cấp lương thực
- Theo dõi dịch bệnh

---

## Bài tập luyện tập

### Bài 1: Đọc biểu đồ

Biểu đồ số học sinh các lớp:
- Lớp 3A: 30 em
- Lớp 3B: 28 em
- Lớp 3C: 32 em

Trả lời:
a) Lớp nào có nhiều học sinh nhất?
b) Tổng cộng có bao nhiêu học sinh?

### Bài 2: Vẽ biểu đồ

Số cây trồng được:
- Thứ Hai: 5 cây
- Thứ Ba: 8 cây
- Thứ Tư: 6 cây
- Thứ Năm: 10 cây

Vẽ biểu đồ cột

### Bài 3: Trắc nghiệm

Biểu đồ cho thấy:
A. So sánh các số liệu
B. Chỉ có một số liệu
C. Không thể so sánh

---

## Góc cha mẹ

### Điều em đang khám phá

Con đang học về **thống kê** - cách hiểu thông tin xung quanh và đưa ra quyết định!

### Gợi ý thảo luận

- Cùng con thu thập dữ liệu về gia đình (tuổi, sở thích...)
- Xem biểu đồ thời tiết trên TV
- Cho con tự vẽ biểu đồ về điểm học tập

### Kỹ năng phát triển

- Thu thập thông tin
- Phân tích dữ liệu
- Đưa ra kết luận
