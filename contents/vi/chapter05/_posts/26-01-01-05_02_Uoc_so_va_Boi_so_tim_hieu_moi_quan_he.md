---
layout: post
title: 04-03 Ước số và Bội số - Tìm hiểu mối quan hệ
chapter: '05'
order: 3
owner: Math Lover Team
lang: vi
categories:
- chapter05
lesson_type: required
---
Trong bài học này, chúng ta sẽ khám phá mối quan hệ thú vị giữa các con số - ước số và bội số!
![Uoc so va Boi so]({{ site.baseurl }}/img/chapter_img/chapter05/02_factors_multiples.svg)


![Trục số — thứ tự các số](https://upload.wikimedia.org/wikipedia/commons/thumb/c/c0/Number_line.svg/960px-Number_line.svg.png)

*Ảnh: Wikimedia Commons — sơ đồ toán học*

---

## Mục tiêu

Trong bài học này, em sẽ:

- Hiểu khái niệm ước số
- Hiểu khái niệm bội số
- Tìm ước số và bội số của một số

---

## Kiến thức đã biết

Em đã biết nhân và chia các số. Hôm nay, chúng ta sẽ học về mối quan hệ giữa các phép tính này nhé!

![Sàng Eratosthenes — lọc số nguyên tố](https://upload.wikimedia.org/wikipedia/commons/e/eb/Sieve_of_Eratosthenes.gif)

*Ảnh: Wikimedia Commons — sơ đồ toán học*


---

## Khám phá toán học

### Ước số là gì?

**Ước số** của một số là số chia hết cho số đó.

**Ví dụ:** Tìm ước số của 12
- 12 ÷ 1 = 12 → 1 là ước của 12
- 12 ÷ 2 = 6 → 2 là ước của 12
- 12 ÷ 3 = 4 → 3 là ước của 12
- 12 ÷ 4 = 3 → 4 là ước của 12
- 12 ÷ 6 = 2 → 6 là ước của 12
- 12 ÷ 12 = 1 → 12 là ước của 12

**Ước số của 12:** 1, 2, 3, 4, 6, 12

### Bội số là gì?

**Bội số** của một số là kết quả của phép nhân số đó với một số tự nhiên.

**Ví dụ:** Tìm bội số của 5
- 5 × 1 = 5 → 5 là bội của 5
- 5 × 2 = 10 → 10 là bội của 5
- 5 × 3 = 15 → 15 là bội của 5
- 5 × 4 = 20 → 20 là bội của 5
- 5 × 5 = 25 → 25 là bội của 5

**Bội số của 5:** 5, 10, 15, 20, 25, ...

---

## Hoạt động khám phá


<!-- vi-interactive:start -->
{% include vi-interactive-lesson.html %}
<!-- vi-interactive:end -->
### Hoạt động 1: Tìm ước số
Tìm tất cả ước số của:
- 6: 1, 2, 3, 6
- 8: 1, 2, 4, 8
- 10: 1, 2, 5, 10


![Bàn tính — cộng trừ nhanh](https://upload.wikimedia.org/wikipedia/commons/thumb/f/f1/Abacus.svg/960px-Abacus.svg.png)

*Ảnh: Wikimedia Commons — sơ đồ toán học*

### Hoạt động 2: Tìm bội số

Viết 5 bội số đầu tiên của:
- 3: 3, 6, 9, 12, 15
- 4: 4, 8, 12, 16, 20
- 7: 7, 14, 21, 28, 35

### Hoạt động 3: Tìm số chung

Tìm các số vừa là ước của 12 vừa là ước của 18?

- Ước của 12: 1, 2, 3, 4, 6, 12
- Ước của 18: 1, 2, 3, 6, 9, 18
- **Số chung:** 1, 2, 3, 6

---

## Toán học qua các thời đại

### 🕰️ Ước số trong lịch sử

**Euclid** (300 TCN) - nhà toán học Hy Lạp - đã nghiên cứu về ước số và tìm ra cách tìm ước chung lớn nhất!

**Sàng Eratosthenes** (276-194 TCN) - phương pháp tìm số nguyên tố bằng cách loại bỏ các bội số.

### 📜 Ứng dụng thực tế

**Trong kiến trúc:** Người xưa dùng ước số để thiết kế các ô cửa có tỷ lệ đẹp!

**Trong âm nhạc:** Nốt nhạc được chia theo bội số của 2 để tạo ra các cao độ hài hòa!

---

## Bài tập luyện tập

### Bài 1: Tìm ước số

a) Ước số của 15
b) Ước số của 20
c) Ước số của 24

### Bài 2: Tìm bội số

Viết 5 bội số đầu tiên của:
a) 6
b) 8
c) 9

### Bài 3: Trắc nghiệm

Số nào vừa là ước của 12 vừa là ước của 18?
   A. 4   B. 6   C. 8


---

## Video tham khảo

Xem thêm trên YouTube để củng cố bài học:

1. [Ước chung lớn nhất — Math Antics](https://www.youtube.com/watch?v=CUEOL3_Wm3Y) — Math Antics
2. [Ước chung bằng phân tích thừa số — Math Antics](https://www.youtube.com/watch?v=SXPsfr-Fnu4) — Math Antics
3. [Mẫu hình số — Math Antics](https://www.youtube.com/watch?v=vV7C7bXm4VI) — Math Antics

*Video: YouTube — kênh giáo dục; nội dung phù hợp lứa tuổi 8–10*


## Góc cha mẹ

### Điều em đang khám phá

Con đang học về **ước số và bội số** - nền tảng quan trọng của số học!

### Gợi ý thảo luận

- Cùng con tìm ước số của các số trong cuộc sống
- Luyện tập nhẩm bội số của các số nhỏ
- Chơi trò chia đều đồ vật để hiểu ước số

### Kỹ năng phát triển

- Tư duy logic
- Phân tích số
- Giải quyết vấn đề
